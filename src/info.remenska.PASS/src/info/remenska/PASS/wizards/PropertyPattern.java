...
public abstract class PropertyPattern {
	...
	public void afterQ(Model m, Collaboration coll, Interaction inter){
		...
		eventQ.setMessageSort(eventQ.getMessageSort());

		if (eventQ.getMessageSort().getLiteral().equals("reply")) {
			String[] returnParams = QuestionTreePage.traceLineMap.get(QuestionTreePage.textStartEvent).getReturnParams();
			for(String param:returnParams){
				OpaqueExpression mh = (OpaqueExpression) eventQ.createArgument("param", null,UMLPackage.Literals.OPAQUE_EXPRESSION);
				mh.getBodies().add(param);
			}
			
		} else {
			String[] params = QuestionTreePage.traceLineMap.get(QuestionTreePage.textStartEvent).getParameters();
...
public void beforeR(Model m, Collaboration coll, Interaction inter){
		...
		// // //// /// / //// / /// 
				...
				if (eventR.getMessageSort().getLiteral().equals("reply")) {
					String[] returnParams = QuestionTreePage.traceLineMap.get(QuestionTreePage.textEndEvent).getReturnParams();
					for(String param:returnParams){
						OpaqueExpression mh = (OpaqueExpression) eventR.createArgument("param", null,UMLPackage.Literals.OPAQUE_EXPRESSION);
						mh.getBodies().add(param);
					}
					
				} else {
					String[] params = QuestionTreePage.traceLineMap.get(QuestionTreePage.textEndEvent).getParameters();
					for(String param:params){
						OpaqueExpression mh = (OpaqueExpression) eventR.createArgument("param", null,UMLPackage.Literals.OPAQUE_EXPRESSION);
						mh.getBodies().add(param);
					}
