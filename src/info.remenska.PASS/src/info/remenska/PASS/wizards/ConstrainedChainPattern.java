...
public void constrained_response_chain(Model m, Collaboration coll, Interaction inter){
	...
	private void constrained_absence(Model m, Collaboration coll,
		...
		//add the parameter values, as specified in PASS, to the message
		if (m1.getMessageSort().getLiteral().equals("reply")) {
			String[] returnParams = QuestionTreePage.traceLineMap.get(textEventX).getReturnParams();
			for(String param:returnParams){
				OpaqueExpression mh = (OpaqueExpression) m1.createArgument("param", null,UMLPackage.Literals.OPAQUE_EXPRESSION);
				mh.getBodies().add(param);
			}
			...
			String[] params = QuestionTreePage.traceLineMap.get(textEventX).getParameters();
			for(String param:params){
				OpaqueExpression mh = (OpaqueExpression) m1.createArgument("param", null,UMLPackage.Literals.OPAQUE_EXPRESSION);
				mh.getBodies().add(param);
			}
